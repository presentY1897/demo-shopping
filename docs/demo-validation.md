...
발급 수는 한 회당 구매1+판매3+환불3=7개다. 기존5회/60초 서버 한도를 유지하며, 성공 발급 시각만 임시 파일에 기록해 다음 발급 가능 시점까지 기다린다. 파일은 토큰·계정 정보를 담지 않고 반복 worker 사이에서도 예산을 유지한다. CI의 재시도1회는 기존 설정이며 안정성 판정은 `--retries=0 --repeat-each=3`로 한다. 고정 sleep, 강제 클릭, API로 UI 쓰기 우회, DB 상태 조작은 사용하지 않는다.
