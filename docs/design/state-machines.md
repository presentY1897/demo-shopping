...
**상태를 쓰는 길은 하나뿐이다.** 결제 확정과 예약 만료도 같은 문을 지난다(`applyWithin`) —
지나지 않는 곳이 하나라도 있으면 「정의되지 않은 전이는 불가능하다」가 거짓이 된다.
...
| `CONFIRMED` | **없음** | 일반 반품은 끝났다. 하자 반품은 관리자 개입(M10)이지 이 표의 화살표가 아니다 |
