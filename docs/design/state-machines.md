...
부분 취소·부분 반품 시 `SellerOrder` 상태는 남은 항목 기준으로 유지된다. 확정된 취소 수량을 제외한 나머지가 모두 검수 합격 반품되면 `RETURNED`로 종료한다. 취소 신청/거절 수량과 검수 전 반품 수량은 완료 수량으로 세지 않는다.
...
주문서 생성은 variant id 순 잠금·가용 재고 조건부 일괄 갱신·예약 생성을 같은 transaction에 둔다. 원본 장바구니 정보를 예약 생성 시 함께 기록한다. 주문 생성은 판매자 주문/항목/최초 상태 이력을 각각 일괄 INSERT하며 기존 주문서 멱등성과 요율·쿠폰·배송 스냅샷 경계를 유지한다(TASK-0133).
