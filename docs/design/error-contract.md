...
| `SEARCH_UNAVAILABLE` | 503 | — | — | 검색을 준비하고 있어요. 잠시 후 다시 시도해 주세요. | 준비 중 안내 + 자동 재시도 (`apps/shop`) |
...
**`SEARCH_UNAVAILABLE` 이 503 인데 `SERVICE_UNAVAILABLE` 이 아닌 이유.** 검색 엔진은 API 와 따로 잠들고
따로 재기동한다. 엔진에 **닿지 못한** 것(연결 실패 · 시한 초과 · 관문이 답한 502 · 503 · 504)은 기다리면
풀리므로 화면은 실패 문구 대신 준비 중 안내를 띄우고 스스로 다시 확인한다. 상태에서 파생된
`SERVICE_UNAVAILABLE` 은 「설정되지 않았다」처럼 기다려도 풀리지 않는 503 과 같은 코드라, 화면이 기다릴지
포기할지를 가를 수 없다. 반대로 엔진이 **답한** 거절(키 불일치 등 Meilisearch 의 `code` 가 실린 응답)은
`INTERNAL_ERROR` 500 이다 — 기다려서 풀리지 않는 것을 기다리게 하지 않는다. `/search` · `/search/suggest` 가
이 코드를 내고, `/search/filters` 는 엔진을 부르지 않아 해당이 없다. 이 응답에는 `Cache-Control: no-store` 가
붙는다.
...
않았습니다" 도, 검색 엔진에 닿지 못했다는 `SEARCH_UNAVAILABLE` 도 결함이 아니라 상태이고, 지우면
호출자에게 남는 것이 없다. 그래서 503 은 문장과 도메인 코드를 그대로 싣는다.
