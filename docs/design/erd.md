...
DB 가 쥔 규칙은 위반 INSERT 를 실제로 시도해 거부되는 것으로 확인한다. 애플리케이션으로 넘어온 셋 중 **계통 key 유일만 경합이 있다** — 나머지 둘은 요청 하나만 보고 답할 수 있는 판정이라 두 요청이 동시에 통과할 여지가 없다. 그래서 동시성 스펙과 음성 대조군을 갖는 것은 그 규칙 하나다.
