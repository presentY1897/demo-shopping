...
**수수료율은 basis point 정수**(1bp = 0.01%, 10000 = 100%). 금액이 정수(원)인데 비율만 부동소수면 정산에서 `0.1 + 0.2` 류의 오차가 다시 들어온다. bp 는 국내 커머스 수수료가 쓰는 해상도(3.5%, 12.75%)를 정확히 표현하면서 정수로 남는다. 범위(0~10000)는 DTO 가 아니라 DB 가 강제한다 — 정산이 이 값을 곱하므로, 음수나 100% 초과가 들어가면 판매자에게 주문액보다 많은 돈이 계산되고 아무도 눈치채지 못한다. 카테고리 기본율과의 우선순위는 `docs/design/pricing.md` 6장.
...
점검은 **고치지 않는다** (TASK-0051 R2). 원인을 모르는 채 값을 고치면 문제가 숨는다 — 검출하고
기록하는 것까지가 잡의 일이고, 보정은 사람의 판단이다.
...
| `SellerOrder` | 판매자별 주문 | **배송·취소·정산 단위**. 상태가 여기에 붙는다. `trackingNumber` 는 **`Shipment` 의 파생 사본**이다 — 상태 머신의 잠금 문장이 한 표만 읽어야 해서 남겼고, 복합 외래키 `(id, trackingNumber) → Shipment(sellerOrderId, trackingNumber)` 가 사본이 갈라지지 못하게 막는다 (TASK-0061 4.2) |
| `Shipment` | 배송 | `SellerOrder` 에 **1:1**(유니크). 「1:N 으로 두고 규율로 1건」은 아무것도 지켜 주지 않는다 — 둘이 생기면 「이 주문이 배송완료인가」에 답이 둘이 된다. 분할 배송은 그 인덱스를 푸는 마이그레이션 하나다 |
| `ShipmentTrackingEvent` | 추적 이력 | 집화·간선·배송출발·배송완료. **가상이다** — 운송사·장소명 전부 지어낸 것이고 번호는 `DEMO-` 로 시작한다 |
| `OrderItem` | 주문 항목 | 상품 스냅샷(JSONB), 단가, 수량, 항목별 할인 안분액 |
| `Shipment` | 배송 | 운송사·운송장번호는 가상 생성. 상태 전이도 가상 진행 |
| `OrderStatusHistory` | 상태 이력 | 이전/이후 상태, 사유, **처리 주체**(`OrderActor`). 기본값을 두지 않았다 — 주체를 말하지 않은 이력이 `SYSTEM` 으로 기록되면 그것은 「모른다」가 아니라 **거짓**이다 (TASK-0059) |
