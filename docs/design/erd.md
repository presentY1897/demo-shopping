...
옵션 값의 추가는 그 값이 참여하는 조합의 Variant 생성, 삭제는 그 조합의 Variant 비활성화다. **옵션 축(색상·사이즈라는 축) 자체의 추가·삭제는 지원하지 않는다** — 조합의 원소 수가 달라져 기존 Variant 의 정규형이 전부 무효가 되고, 주문 이력이 걸린 뒤에는 되돌릴 수 없다. 초안 전체 교체는 편집기 저장 계약(TASK-0113)이 다룬다.
...
`ProductVariant.stock` 은 **현재값**이고 원장이 그것을 설명한다. 원장이 붙을 수 있으려면 Variant 의 id 가 안정적이고 사라지지 않아야 하며(2장 — 소프트 삭제), 조건부 갱신이 뚫렸을 때를 대비해 `stock >= 0` 을 DB 가 쥐고 있어야 한다. 둘 다 상품 스키마 쪽에 이미 있다.
