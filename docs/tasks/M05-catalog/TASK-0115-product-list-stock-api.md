...
측정은 전부 실제 PostgreSQL 이고, 응답은 `createApiClient` 를 지나므로 `packages/shared` 의
스키마와 어긋나면 단언 이전에 실패한다. 괄호 안은 잰 스펙 파일이다.

| # | 기준 | 측정 방법 | 목표 | 실측 | 충족 |
| --- | --- | --- | --- | --- | --- |
| F1 | 소유 필터 | 두 스토어의 상품이 있는 DB 에서 목록 조회 (`products-list`) | 자기 상품만, 남의 id 직접 조회 시 403 | 목록에 자기 1건만. 남의 상품 id 로 `GET /seller/products/:id/variants` · `POST …/duplicate` 둘 다 **403 `FORBIDDEN`**. 스토어 없는 운영자는 목록 자체가 403 | [x] |
| F2 | 재고 조정 | 재고 12 에 `+5 INBOUND` (`stock-adjust`) | `stock` 5 증가 **그리고** `StockLedger` 1행 | 12 → **17**, 원장 행 **+1**, `sum(quantity)` = `stock` = 17, `actorId` = 요청자 | [x] |
| F3 | 동시성 | 콘솔 조정 1건과 `SALE` 3건 동시, 10회 (`stock-adjust-contention`) | 최종 재고 = 원장 합계, 음수 0건 (A7) | 10회 전부 재고 **12** = 합계, `seq` **1..5 빈틈 0**, 사슬 어긋남 0, `balanceAfter < 0` **0건** | [x] |
| F4 | 재고 필터 | `stock=out` · `stock=low` (`products-list`) | 각각 총재고 0, 총재고 1~`LOW_STOCK_THRESHOLD`. 겹치지 않고 배지와 같은 술어 | `out` → 재고 0 인 1건만. `low` → 경계 **1 과 5 를 포함**하고 0 과 6 은 제외. **배지가 붙은 행의 집합 = `low` 필터의 결과**. 순수 모듈 스펙이 0..19 전 구간에서 둘을 대조하고 교집합이 빈 것까지 본다 | [x] |
| F5 | 일괄 상태 변경 | 5개 id 로 판매중지 (`products-list`) | 5건 전부 반영, 남의 상품 id 를 섞으면 403 (부분 적용 없음) | 5건 전부 `INACTIVE`. 남의 id 를 섞으면 **403 이고 앞의 5건은 `DRAFT` 그대로**. 없는 id 를 섞으면 404 이고 역시 그대로. 뒤 상품이 판매 불가면 **400 `PRODUCT_NOT_SELLABLE` 이고 `ACTIVE` 0건** — 검사 선행만으로는 못 막는 쪽을 트랜잭션이 되돌린다 | [x] |
| F6 | 페이지네이션 | 100건을 `limit=7` 커서로 끝까지 (`products-list`) | 중복 0건 · 누락 0건 | 15페이지에 **100건**, 고유 id **100개** | [x] |
| F7 | 복제 | 옵션 2종(2×2) 상품 복제 (`products-list`) | 옵션 · Variant 동수, 상태 `DRAFT`, 재고 0, 원장 0행 | 옵션 **2** · Variant **4** 동수, `DRAFT`, 전 Variant 재고 **0**, 복제본의 원장 **0행**, 원본 SKU 와 겹치는 것 **0개**, 조합별 가격 유지 | [x] |
| F8 | 원장 우회 금지 | `test/db/stock-single-path.spec.ts` (TASK-0036) | `product_variant.stock` 을 직접 `UPDATE` 하는 경로 0건 | `apps/api/src` 전 파일 대상, 이 TASK 가 더한 4개 포함해 **0건**. 조정은 `StockService.adjust`, 복제의 초기 재고는 `StockService.open` 을 지난다 | [x] |
| F9 | 이력 조회 | 초기 재고 0 인 Variant 에 조정 3회 후 `GET /variants/:id/ledger` (`stock-adjust`) | 3행 + `balanceAfter` 순서대로 일치, `seq` 1..3 | `+7 / −2 / +5` → 3행, `seq` **3·2·1**(최신순), `balanceAfter` **10·5·7**, 응답 머리의 `stock` = `ledgerBalance` = 10. 조정이 돌려준 `seq` 로 방금 쓴 행을 찾는 것까지 확인 | [x] |
| F10 | 판매자 상태 | `SUSPENDED` 스토어 (`products-list` · `stock-adjust`) | 403, 목록 조회는 200 | 목록 **200**(1건), 이력 조회 **200**, 재고 조정 **403 `PRODUCT_SELLER_INACTIVE`**, 일괄 상태 변경 **403 같은 코드**. 조정 실패 후 재고 변화 없음 | [x] |
...
| Q5 테스트 충실도 | **라인 커버리지 80%**(M05 부터). 품절 임박 판정 · 복제 조립 · 스토어 상태 게이트는 순수 로직이므로 **분기 100%**(Q5 강화) | 대역: **실제 PostgreSQL** |
...
**1장 실측** — `pnpm typecheck && pnpm lint && pnpm build && pnpm test:coverage`, 전부 통과.

| # | 목표 | 실측 |
| --- | --- | --- |
| Q1 | error 0 | **0** |
| Q2 | error 0 · warning 0 | **0 / 0** |
| Q3 | 전 앱 성공 | api · shop · seller · admin · shared · ui · api-mocks 전부 성공 |
| Q4 | 전부 통과 | **3,447 통과 · 1 스킵**(api 1,677 · ui 843 · admin 336 · shop 234+1 · seller 178 · api-mocks 179). 이 TASK 가 더한 것 **106개** |
| Q5 | `apps/api` 라인 80% 이상 | **96.24%** (구문 94.33% · 분기 84.60% · 함수 98.18%) |
| Q5 강화 | 순수 로직 분기 100% | `seller-product-filters.ts` · `product-duplicate.ts` · `store-write-gate.ts` **전부 100%** (구문 · 분기 · 함수 · 라인). `vitest.config.mjs` 에 임계치로 고정 |

| # | 기준 | 측정 방법 | 목표 | 실측 | 충족 |
| --- | --- | --- | --- | --- | --- |
| A1 | 응답 시간 | 각 50회 p95 (`products-list-performance`) | 300ms 이하 | 100건 목록 **3.3ms** · 필터+검색 20건 **2.5ms** · 12행 조합 표 **5.6ms** · 재고 조정(행 잠금 포함) **12.6ms** | [x] |
| A2 | 입력 검증 | 잘못된 입력 (`products-list` · `stock-adjust`) | 400 + 통일된 에러 포맷 | `delta: 0` → 400 · 알 수 없는 `status` → 400 · 알 수 없는 `stock` 필터 → 400 · `type: 'SALE'` → 400(`details[].field` = `type`) · 사유 없는 `ADJUST` → 400(`field` = `reason`) · `INBOUND` 에 음수 → 400. 재고 부족은 409 이고 재고는 그대로 | [x] |
| A3 | 권한 | `BUYER` 토큰 | 403 | 목록 **403** · 재고 조정 **403** | [x] |
| A4 | 인증 | 토큰 없이 | 401 | 목록 · 재고 조정 둘 다 **401 `AUTH_REQUIRED`** | [x] |
| A5 | N+1 | 쿼리 로깅을 켠 실제 `PrismaClient` 로 문장 수 (`products-list-performance`) | 상품 수와 무관하게 일정 | 목록 5건 **1문장** · 100건 **1문장**, 필터 셋과 검색을 켜도 **1문장**. 조합 표 1조합 **2문장** · 12조합 **2문장**. 일괄 상태 변경은 `2n+1`(기울기 **2**, 2건 5문장 · 8건 17문장) — 상품당 고정 | [x] |
| A6 | 실 DB | 서비스·API 테스트 실행 대상 | Prisma 모킹 0건 | **0건**. 성능 스펙이 바꿔 끼우는 것도 같은 워커 DB 를 보는 진짜 `PrismaClient` 이고 차이는 쿼리 로깅뿐이다 | [x] |
| A7 | 동시 요청 | F3 + 재고보다 많은 요청 (`stock-adjust-contention`) | 초과 차감 · 중복 처리 0건 | 재고 1 에 동시 5건 → **1건 성공 · 4건 409**, 재고 0, 합계 = 재고, 10회 반복 전부. 재고 3 에 5건 → **3 성공 · 2건 409**. 요청이 실제로 행 잠금에서 대기하는 것을 `pg_stat_activity` 로 관측. **음성 대조군**: 절대값 입력이면 그 사이 판매가 10/10 회 지워지고(재고 17, 정답 14) 그때도 원장은 10/10 회 정합적이다 — 같은 순서를 delta 로 말하면 10/10 회 14 | [x] |
| C1 | 스키마 단일 출처 | 응답 DTO 출처 확인 | 전부 `packages/shared` 의 zod 스키마 | 새 스키마 9종 + 상수 1개가 전부 `packages/shared/src/api/{products,stock}.ts`. `apps/api` 에 응답 타입 재정의 **0건** | [x] |
| C3 | 실제 응답 검증 | 통합 테스트에서 실제 응답을 `parse` | 전부 통과 | `createApiClient` 가 모든 응답을 스키마로 파싱하므로 구조적으로 성립하고(어긋나면 `malformed_response`), `sellerProductListResponseSchema` · `sellerVariantListResponseSchema` · `productBulkStatusResponseSchema` · `stockAdjustResponseSchema` 는 스펙에서 명시적으로도 `parse` 한다 | [x] |

### 6.3 문서

| # | 기준 | 실측 | 충족 |
| --- | --- | --- | --- |
| D1 | 상태를 `완료` 로 변경. 두 인덱스는 오케스트레이터가 갱신한다 | 이 문서의 상태를 `완료` 로 바꿨다. `docs/tasks/README.md` · `M05-catalog/README.md` 는 **손대지 않았다** — `main` 에서 계속 움직이는 파일이라 작업자가 고치면 매번 충돌한다(CLAUDE.md 2장) | [x] |
| D2 | 품절 임박 기준을 설계 문서에 명시 | `docs/design/erd.md` 3장에 "품절과 품절 임박은 하나의 술어가 정한다" 를 더했다 — 세 단계 표, 겹치지 않는 이유, 예약(M07)이 붙으면 바뀌는 지점 | [x] |
| D3 | 결정 변경 없음 확인 | 없다. D-024(조정량 입력)를 **따랐고**, 이 TASK 의 음성 대조군이 그 결정의 근거를 실측으로 남긴다 | [x] |
| D4 | 새 환경변수 없음 확인 | 없다 | [x] |
| D5 | 새 라이브러리 없음 확인 | 없다 (8장) | [x] |

## 7. 리스크 / 열린 질문

| # | 내용 | 대응 | 결과 |
| --- | --- | --- | --- |
| R1 | 목록에서 Variant 를 모두 로드해 N+1 이 난다 | 목록은 **집계값(총재고 · 최저가)만** 반환하고 Variant 는 상세 엔드포인트로 분리한다(A5) | **닫힘** — 5건과 100건 모두 1문장. 조합 표도 1조합과 12조합이 같은 2문장 |
| R2 | 재고를 직접 `UPDATE` 하는 우회 경로가 생긴다 | 조정은 TASK-0036 의 `StockService` 한 곳만 호출한다. F8 이 코드 검색으로 고정한다 | **닫힘** — 새 파일 4개를 포함해 0건 |
| R3 | 품절 임박 기준값 5 가 카테고리마다 부적절할 수 있다 | 상수 하나로 시작한다. 카테고리별 기준이 필요해지면 그때 설정값으로 승격하고, 상수를 읽던 곳만 바꾸면 된다 | **열려 있음(의도)** — 승격 지점은 `LOW_STOCK_THRESHOLD` 와 `stockBandOf` 둘뿐이다. 판정을 서버가 하고 화면은 `isLowStock` 을 그리기만 하므로, 카테고리별 기준이 오면 화면은 손대지 않는다 |
| R4 | 일괄 상태 변경에 남의 상품 id 가 섞이면 부분 적용된다 | 소유권 검사를 **먼저 전부** 수행하고 하나라도 실패하면 403 으로 끝낸다(F5) | **닫힘, 다만 검사 선행만으로는 부족했다** — `PRODUCT_NOT_SELLABLE` 은 `settle` 안에서만 알 수 있어 검사 단계에서 걸러지지 않는다. 전체를 한 트랜잭션에 넣어 되돌린다 |
| R5 | 복제된 상품의 SKU 가 중복된다 | 복제 시 SKU 에 접미사를 붙이거나 비운다 | **닫힘 — "비운다" 쪽** . 접미사는 규칙이 두 벌(생성기 · 복제기)이 되고, TASK-0113 의 기본 규칙(시간 접두사 + 난수 꼬리 6자리)이 이미 충돌하지 않는 이름을 만든다 |

### 이 TASK 가 남기는 것

| # | 내용 | 받는 곳 |
| --- | --- | --- |
| R6 | **일괄 상태 변경에는 낙관적 잠금이 없다.** 상태 한 칸을 방금 본 행들에 적용하는 일이라 `version` 을 요구하지 않기로 했다(4장). 목록에서 체크한 상품을 그 사이 편집기가 발행했다면 마지막 요청이 이긴다 | 화면이 이것을 어떻게 보이는지는 **TASK-0116** 의 판단이다. 서버는 바뀐 행을 목록과 같은 모양으로 돌려주므로 즉시 갱신할 수 있다 |
| R7 | **이름 검색은 색인을 타지 않는다.** 앞에 와일드카드가 붙는 `ILIKE` 라서 그렇고, 한 스토어의 카탈로그로 항상 좁혀지기 때문에 100건에서 p95 2.5ms 다 | 스토어 하나가 수천 건이 되면 **M06(Meilisearch)** 이 이미 그 문제를 맡는다. 지금 색인을 더하면 쓰이지 않는 색인이 쓰기 비용만 늘린다 |
...
| 2026-09-05 | 구현 완료. 6장을 실측값으로 채우고 상태를 `완료` 로 바꿨다. 인덱스 두 곳은 오케스트레이터 몫 |
