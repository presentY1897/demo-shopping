...
TASK-0111 에서 별도 퍼미션(`profile.write` · `profile.delete`)을 받는다. `authorize.spec.ts` 가 이 조건을 테스트로 고정한다.
...
| F5 | 데모 상호작용 | 판매자 데모 신청 → 관리자 데모 승인 전 과정 | 전 과정 성공 | 입점 신청 플로가 아직 없음 → **TASK-0108 에서 검증** | [ ] |
...
F4 는 도메인 엔드포인트가 아직 없어(입점 승인은 TASK-0108, 카탈로그는 M06) **`seller.approve` ·
...
| R4 | 퍼미션 목록에 `claim.write`(구매자의 반품 신청)·`seller.write`(판매자의 스토어 정보 편집)가 없다 | 해당 엔드포인트가 아직 없어 **지금은 구멍이 아니다**. 만드는 TASK(M08 · **TASK-0108**)가 퍼미션을 추가하고 매트릭스를 다시 생성한다 — `seller.write` 는 TASK-0108 이, 자기 프로필 편집용 `profile.write` · `profile.delete` 는 TASK-0111 이 추가한다. 목록을 미리 늘리면 아무도 쓰지 않는 권한이 표에 남는다 |
...
| 2026-09-03 | D-208 레이어 분할에 따라 본문 참조 갱신 — 입점 플로는 TASK-0026 → **TASK-0108**, 자기 프로필 편집 퍼미션은 TASK-0027 → **TASK-0111**. 위 2026-09-03 이력 행에 적힌 TASK-0026 은 분할 전 번호다 |
