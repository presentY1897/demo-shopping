...
- [x] Google 계정으로 로그인하면 사용자가 생성되거나 조회된다 (F1 · F2)
- [x] 신규 사용자에게 `BUYER` 역할이 같은 트랜잭션에서 부여된다 (F1)
- [x] `state` 검증에 실패하면 로그인이 거부된다 (F3)
- [x] 세 앱이 각각 **자기 오리진으로** 복귀한다 (콜백 URL 자체는 API 하나다) (F4)
- [x] 복귀 대상은 허용 목록에서 고른다 — 임의 주소로 리다이렉트되지 않는다 (F10)
- [x] 사용자가 동의를 취소하면 오류 화면 없이 로그인 페이지로 돌아간다 (F5)
- [x] Google 자격증명이 없는 환경에서도 API 가 정상 기동한다 (해당 엔드포인트만 503) (F8 · F9 · F11)
...
| # | 기준 | 측정 방법 | 목표 | 충족 |
| --- | --- | --- | --- | --- |
| F1 | 최초 로그인 | 신규 `googleSub` 로 콜백 1회 | `User` 1행 + `UserRole(BUYER)` 1행, `lastLoginAt` 갱신 | [x] |
| F2 | 재로그인 | 같은 `googleSub` 로 콜백 재호출 | User 행 수 불변, 역할 중복 부여 0건 | [x] |
| F3 | CSRF 방지 | ① state 쿼리 변조 ② 쿠키 없이 콜백 ③ 같은 state 두 번 | 셋 다 거부, User 생성 0건. **①은 Google 호출 이전에 끊긴다** | [x] |
| F4 | 앱별 복귀 | `app=shop\|seller\|admin` 각각 로그인 | `Location` 이 각 앱 오리진, 셋이 서로 다름. 콜백 URL 은 셋 다 **하나** | [x] |
| F5 | 취소 처리 | 콜백에 `?error=access_denied` | 302 `.../login?status=cancelled`, `reason` 없음, 봉투 응답 0건 | [x] |
| F6 | 권한 없는 앱 | BUYER 만 있는 계정으로 `app=admin` | `Location` 에 `notice=no_role`. 역할을 주면 사라진다. **표시는 TASK-0023** | [x] |
| F7 | 동시 최초 로그인 | 같은 `googleSub` 로 콜백 2건 동시 | User 1행만 생성, 둘 다 같은 사용자로 성공 (A7). 아래 참조 | [x] |
| F8 | 미설정 기동 | 두 환경변수 없이 부팅 후 호출 | 기동 성공(`/health` 200), 두 엔드포인트만 503 | [x] |
| F9 | 반쪽 설정 | `GOOGLE_CLIENT_ID` 만 두고 해석 | `issues` 에 빠진 변수 이름, `config: null` → 부팅 거부. **값은 담기지 않는다** | [x] |
| F10 | open redirect | 허용 목록에 없는 앱으로 authorize | 400, `Location` 헤더 자체가 없음 | [x] |
| F11 | 설정이 사라진 뒤의 콜백 | 자격증명 없이 재배포된 뒤 유효한 state 로 콜백 | `reason=not_configured` — `exchange_failed` 로 뭉개지 않는다 | [x] |

> **F7 은 두 곳에서 증명한다.** 통합 스펙은 경합에서 진 쪽이 500 이 아니라 이긴 쪽의 행을 들고
> 돌아오는지를 본다. 다만 두 요청이 실제로 겹쳤는지는 런타임에 달려 있어 그것만으로는 **줄을 섰어도
> 통과**한다 — QUALITY-GATES A7 이 경고하는 모양이다. 그래서 경합 자체는
> `test/db/google-identity-contention.spec.ts` 가 `barrier` 로 순서를 고정해 증명하고, **음성
> 대조군**(부분 유니크 인덱스 없는 테이블)이 확인 후 생성만으로는 한 사람에게 계정이 둘 생기는 것을
> 보인다. 대조군이 초록이 되면 본 검사가 아무것도 증명하지 않고 있다는 뜻이다.
>
> **F11 은 원래 없던 기준이다.** 복귀 계약에 `not_configured` 를 넣어 놓고 그 경로를 만들지 않아
> 도달 불가능한 값이었다. 두 홉 사이에 자격증명이 사라지는 창(재배포·키 회전)이 실제로 있어 경로를
> 만들고 기준을 추가했다.
...
| Q3 | `pnpm build` | 전 앱 성공 | 성공 | [x] |
| Q4 | `pnpm test` | `apps/api` **1,244개 통과** | 전부 통과 | [x] |
| Q5 | 커버리지 | 전역 라인 80% 통과. `google-config` · `app-origins` · `oauth-state` **분기 100%** | 80% / 100% | [x] |
| A1 | 응답 시간 p95 | authorize **1.8ms** · 재로그인 콜백 **6.1ms** · 최초 로그인 콜백 **8.2ms** | 300ms 이하 | [x] |
| A2 | 입력 검증 | 알 수 없는 `app` · `app` 누락 → 400 | 400 + 통일 포맷 | [x] |
| A6 | 실 DB | 통합·경합 스펙 전부 실제 PostgreSQL. Prisma 모킹 0건 | 모킹 0 | [x] |
| A7 | 동시 요청 | `barrier` 로 순서 고정 + 음성 대조군 | 중복 처리 0건 | [x] |
| C1 | 스키마 단일 출처 | 복귀 계약 어휘가 `packages/shared/src/api/google-auth.ts` 한 곳 | 한 곳 | [x] |

**A1 이 재지 않는 것** — Google 왕복 두 번은 포트 뒤에서 즉시 답하므로 측정에서 빠진다. 우리 것이
아니고 어떤 임계치도 그것을 붙잡을 수 없다. 남는 것이 이 TASK 가 책임지는 부분이다.
...
| --- | --- | --- |
| D1 | 상태 갱신 + 인덱스 2곳 | [x] |
| D4 | `GOOGLE_CLIENT_ID` · `GOOGLE_CLIENT_SECRET` 를 `.env.example` 과 배포 대조표에 반영 | [x] |
| D6 | 프리뷰 OAuth(R3)를 `docs/HANDOFF.md` 3.2 에 주인 미정으로 남긴다 | [x] |
...
| 2026-09-04 | **완료.** F11(설정이 사라진 뒤의 콜백) 추가 — `not_configured` 가 계약에만 있고 도달 불가능한 값이었다. Google 호출에 10초 마감 추가 — 없으면 콜백 요청이 그만큼 열려 있는다. 단위 스펙을 쓰다 나온 도달 불가능한 분기 셋(`split(...)[0] ?? null`, `?? ''`, `localPart` 의 대체값)을 코드 쪽에서 없앴다 — 임계치를 낮추면 진짜 미달을 못 본다 |
