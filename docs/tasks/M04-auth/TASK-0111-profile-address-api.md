...
| # | 기준 | 측정 방법 | 목표 | 실측 | 충족 |
| --- | --- | --- | --- | --- | --- |
| F1 | 프로필 수정 | `PATCH /me` 로 이름 변경 후 `GET /me` | 변경값 반환 | 이름·아바타 모두 반영. 아바타는 `null` 로 지울 수 있고 키를 빼면 유지된다. 빈 본문은 무변경 (`me-profile.spec.ts` 5건) | [x] |
| F2 | 배송지 추가 | 3건 생성 후 목록 조회 | 3건 반환, 각 항목이 `addressSchema` 통과 | 3건. `createApiClient` 가 `addressListResponseSchema` 로 파싱하므로 항목 검증은 구조적으로 걸린다 | [x] |
| F3 | 기본 배송지 전환 | 다른 배송지를 기본으로 지정 | 이전 기본 해제, `isDefault=true` 가 정확히 1건 | DB 직접 조회로 1건, 대상이 새 배송지. 이미 기본인 것을 다시 지정해도 1건(멱등) | [x] |
| F3b | 기본 배송지 동시 지정 | 서로 다른 두 배송지에 기본 지정을 동시 호출 | 한 건 성공·한 건 실패(409), `isDefault=true` 1건 유지 (A7) | 성공 1 · 실패 1(**409**), 기본 1건. 겹침은 바깥 커넥션이 해제 대상 행을 `FOR UPDATE` 로 잡고 `pg_stat_activity` 에서 둘이 잠금에 걸린 것을 확인한 뒤 놓아 주는 방식으로 **만들어서** 검사한다 | [x] |
| F3c | DB 강제 확인 | `isDefault=true` 인 두 번째 행을 psql 로 직접 INSERT·UPDATE | **DB 가** 거부 (S5) | INSERT·UPDATE 모두 `23505` / `Address_userId_default_key`. "해제 후 지정" 순서는 통과 — 조건이 거꾸로 적힌 경우까지 잡는다 | [x] |
| F3d | 기본 배송지 삭제 | 기본 배송지를 삭제 | 남은 것 중 가장 최근 생성분이 자동 승격, `isDefault=true` 1건 (R3) | 3건 중 기본(가장 오래된 것)을 삭제 → 가장 최근 것이 승격, 기본 1건. 기본이 아닌 것을 지우면 기본 불변. 마지막 한 건을 지우면 기본 0건 | [x] |
| F4 | 밀도 승격 | `PATCH /me/preferences` 로 `MAXIMAL` 저장 후 다른 세션에서 조회 | 같은 값 반환 | 다른 클라이언트 객체(새 fetch·새 쿠키 자)로 조회해도 `MAXIMAL`. 첫 저장에서 행이 생기고, 조회는 행을 만들지 않는다(2회 조회 후에도 0행) | [x] |
| F4b | 설정 기본값 일치 | 설정 행이 없는 계정의 `GET /me/preferences` 와, 빈 행을 SQL 로 넣은 계정의 조회를 비교 | 두 응답이 같고 `DEFAULT_USER_PREFERENCE` 와 일치 | 두 응답 동일, `DEFAULT_USER_PREFERENCE` 와 일치. `GET /me` 가 싣는 설정도 같은 값 | [x] |
| F5 | 탈퇴 | `DELETE /me` 실행 | `Address` 행 0건, `User` 행은 남고 `deletedAt` 이 찍힌다, 그 계정을 참조하는 이력 행(`StockLedger.actorId`)이 남는다, 같은 `googleSub` 로 다시 로그인하면 **다른 user id** 가 나온다 | 배송지 2건 → 0건(`deletedAddresses: 2`), `User.deletedAt` 기록, `StockLedger` 참조 1건 유지, `googleSub` 는 그대로인데 재로그인은 **다른 user id**. 탈퇴 후 `/me`·`/me/addresses` 는 유효한 access 토큰으로도 404, 두 번째 탈퇴도 404 | [x] |
| F6 | 남의 배송지 | 다른 사용자의 배송지 id 로 `PATCH` | 403 (`own` 스코프). `profile.write:any` 를 가진 `ADMIN_SUPER` 도 403 | `BUYER`·`ADMIN_SUPER` 모두 403 (`out_of_scope`). 삭제·기본 지정도 403 이고 대상 계정의 행은 그대로. 없는 id 는 404, UUID 가 아니면 400 | [x] |
| F7 | 알림 설정 | 마케팅 수신 끄기 | 재조회 시 `notifyMarketing=false` | `false`. 한 필드만 보내면 나머지 필드는 그대로 | [x] |
| F8 | 탈퇴 후 세션 | 탈퇴 직후 기존 refresh 토큰으로 갱신 시도 | 401 | 401. 살아 있던 refresh 행 1 → 0. 다른 앱(seller)에 열어 둔 세션도 함께 401 | [x] |
| F9 | 매트릭스 재생성 | `pnpm --filter @shopping/api docs:matrix` 후 `git diff` | 차이 0줄 — `89b1e50` 이 이미 반영했다. `permission-matrix.spec.ts` 통과 | 실행 후 `git status` 깨끗 (0줄). `permission-matrix.spec.ts` 통과 | [x] |
| F10 | `/me` 에는 남의 것을 요청할 방법이 없다 | 라우트 목록에서 사용자 식별자를 받는 경로를 센다 | 0건 — `:id` 는 전부 **자기 배송지** 를 가리킨다 | 두 컨트롤러의 라우트 9개 중 사용자 식별자를 받는 경로 0건. 본문에 남의 `userId` 를 실어도 그 계정은 변하지 않는다 | [x] |
...
| # | 기준 | 측정 방법 | 목표 | 실측 | 충족 |
| --- | --- | --- | --- | --- | --- |
| A1 | 응답 시간 | 배송지 50건 목록 · `GET /me` 를 각각 50회, p95 | 300ms 이하 | 목록 **4.19ms**, `GET /me` **2.83ms** | [x] |
| A2 | 입력 검증 | 우편번호 4자리 · 알 수 없는 밀도값 | 400 + 통일된 에러 포맷 | 400 + `details[].field` = `postalCode` / `density`, `code: INVALID`. 전화번호·통화 코드·빈 이름·비 URL 아바타도 같은 모양. 거절된 요청은 아무 행도 남기지 않는다 | [x] |
| A3 | 권한 | 퍼미션 없는 주체로 `PATCH /me` 호출 | 403 | `ADMIN_OPERATOR` → 403 `profile.write 퍼미션이 없습니다.` 배송지 추가도 403. 같은 계정의 `GET /me` 는 200 (`user.read:any`) | [x] |
| A4 | 인증 | 토큰 없이 호출 | 401 | `GET /me` · `GET /me/addresses` 모두 401 `AUTH_REQUIRED` | [x] |
| A5 | N+1 | 배송지 1건과 50건의 `GET /me/addresses` 쿼리 로그 비교 + `GET /me` 쿼리 수 | 배송지 건수와 무관하게 쿼리 수 일정 | 1건·50건 모두 총 **2문장**, 그중 `Address` **1문장**. `GET /me` 는 총 **3문장**, `Address` **0문장** | [x] |
| A6 | 실 DB | 서비스·API 테스트 실행 대상 | Prisma 모킹 0건, 실제 PostgreSQL | Prisma 모킹 0건. 대체한 것은 `GOOGLE_OAUTH` 포트 하나뿐이고, 쿼리 로그를 켠 쪽도 같은 워커 DB 에 붙는 **진짜** `PrismaClient` 다 | [x] |
| A7 | 동시 요청 | F3b | 기본 배송지 2개 발생 0건 | 0건. HTTP 2건 동시 호출 · DB 레벨 2건 · 5건 동시, 모두 기본 1건. 음성 대조군(인덱스 없는 표)에서는 같은 순서가 기본을 2개 만든다 | [x] |
| S5 | 제약 강제 | F3c | **DB 가** 거부 | `23505` / `Address_userId_default_key`, INSERT·UPDATE 양쪽 | [x] |
| C1 | 스키마 단일 출처 | 응답 DTO 출처 확인 | 전부 `packages/shared` 의 zod 스키마 | 컨트롤러의 반환 타입 8종이 전부 `@shopping/shared` 에서 온다. `apps/api` 는 응답 타입을 하나도 다시 정의하지 않는다 | [x] |
| C3 | 실제 응답 검증 | 통합 테스트가 `createApiClient` 를 통해 호출하므로 모든 응답이 해당 zod 스키마로 `parse` 된다 | 전부 통과 | 전부 통과. 이름이 바뀐 필드는 단언이 언급하지 않아도 `malformed_response` 로 빨개진다 | [x] |

### 6.3 문서

| # | 기준 | 실측 | 충족 |
| --- | --- | --- | --- |
| D1 | 상태를 `완료` 로 변경 + `docs/tasks/README.md` · `M04-auth/README.md` 인덱스 갱신 | 상태 `완료`. **인덱스 두 곳은 오케스트레이터가 갱신한다**(병행 규칙 — 두 README 는 `main` 에서 계속 움직인다) | [x] |
| D2 | 밀도 저장 정책(비로그인 localStorage → 로그인 서버 승격)을 `docs/design/pages.md` 에 반영 | 「표시 밀도 3단계」에 승격 절차·단일 출처·행 지연 생성을 추가 | [x] |
| D3 | 결정 변경 없음 확인 (`DECISIONS.md` 2장과 대조) | 변경 없음. 2장의 퍼미션 기반 RBAC·기본 거부·`own` 스코프를 그대로 쓴다 | [x] |
| D4 | 새 환경변수 없음 확인 | 없음. `.env.example` 무변경 | [x] |
| D5 | 새 라이브러리 없음 확인 (8장) | 없음. `package.json` 무변경 | [x] |

### 6.4 게이트 실행 결과 (2026-09-04, 로컬)

| 게이트 | 명령 | 결과 |
| --- | --- | --- |
| Q1 | `pnpm typecheck` | 8개 프로젝트 통과, error 0 |
| Q2 | `pnpm lint` | 루트 + 8개 프로젝트 통과, error 0 · warning 0 |
| Q3 | `pnpm build` | 전 앱 성공 |
| Q4 | `pnpm test` | 전부 통과. `apps/api` **87 파일 · 1412 테스트** (이 TASK 가 더한 것은 **8 파일 · 77 테스트**) |
| Q5 | `pnpm --filter @shopping/api test:coverage` | 임계치 통과. 라인 **95.76%**(기준 80%), `src/profile/default-address.ts` 는 분기·함수·라인·구문 **100%** |
...
| R3 | 기본 배송지를 삭제하면 기본이 0개가 된다 | 남은 것 중 **가장 최근에 만든 것**(`createdAt` 내림차순, 동률이면 `id` 내림차순)을 자동 승격한다. 규칙은 `src/profile/default-address.ts` 한 곳에 있고 F3d 가 고정한다. 마지막 한 건이었다면 승격 대상이 없으므로 기본도 없다 — 배송지가 0건인 상태에서는 불변식이 "기본은 최대 1개" 로 읽힌다. **남은 틈**: 한 계정에서 삭제 두 건이 동시에 들어와 한쪽이 승격 대상 행을 먼저 가져가면 기본이 잠시 0개가 된다(승격을 `update` 가 아니라 `updateMany` 로 해서 트랜잭션이 깨지지는 않는다). 다음 저장·지정에서 복구되고, 막으려면 주소록 전체를 잠가야 하는데 그러면 잠금 없이 유지하려는 기본 지정 경로와 교착한다 |
...
| 2026-09-04 | **구현 완료.** 6장을 실측값으로 채우고 상태를 `완료` 로 바꿨다. 게이트 4개 통과(`apps/api` 87 파일 · 1412 테스트, 이 TASK 가 더한 것 8 파일 · 77 테스트). 구현 중 정정 한 건 — **`profileSchema` 에서 `isDemo` 를 뺐다.** `apps/api/src/auth/demo-containment.spec.ts`(TASK-0105 F8)가 이 컬럼을 이름으로 부르는 파일을 세 개로 못 박고 있어, 응답 스키마에 넣으면 그 장치가 깨진다. 4장 절단면 절에 이유를 적었다 |
