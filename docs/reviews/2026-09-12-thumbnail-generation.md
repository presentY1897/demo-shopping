...
위 문장은 최초 구현 검증 시점의 상태다. 이후 PR147(main c58910a)과 Render 배포가 성공했고 운영 migration 적용, API/database/search 정상, 세 앱 운영 배포 성공을 확인했다. 사용자가 Render의 `THUMBNAIL_GENERATION=on`, `LOG_LEVEL=log`를 확인했으며 환경 변경 배포6396686768도 성공했다.

기존 이미지2,101건을 읽기 전용으로 분류했다. 운영 CDN의 판매자 소유 raster 경로532건, SVG1,281건, 기타288건이다. 로컬 R2 설정은 운영과 달라 운영 설정으로 사용하지 않았다. PNG 원본1건을 curl HTTP200으로 확인하고 크기/해시를 비공개 운영 기록에 보존한 뒤 해당1건만 영속 큐에 등록했다. 원본 URL과 주문 스냅샷은 수정하지 않았다. 작업은 PENDING/attempts=0으로 남아 있어 실제 생성/R2 PUT 검증은 아직 미완료이며 나머지 등록은 보류한다.

사용자에게 메모리 경고가 보이지 않지만, 기존 경고는 처음1회만 기록하므로 로그 부재만으로 원인을 확정할 수 없다. 작업자 시작 준비/폴링 시작, flag off/저장소 미설정에 따른 미실행 이유를 기록하고 메모리 대기는 최초 및60초마다 availableBytes/requiredBytes를 기록하도록 보완한다. 0은 여유 없음 또는 제한 측정 불가를 뜻한다. 복구 시에도 기록한다. 메모리 한도·동시성·작업 DB 계약은 유지하고 비밀값이나 파일 URL은 로그에 넣지 않는다. 원인은 보완 배포 후 확인한다.
