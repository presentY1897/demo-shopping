...
TASK0131의 hosting scale-to-zero 측정은 별도 미완료 항목이다. 앱 코드/스키마/timeout/JWT/쿠키/이미지 저장소는 이번 이전에서 바꾸지 않았다. 처음에는 문서만 변경했으나 CI 두 번에서 기존 결제 SQL 횟수 검사가27회/상한26회로 실패했다. 조사 결과 응답 이후 Notification 일괄 INSERT가 도착하는 시점에 따라 기존 검사가 이를 빠뜨렸다. 기존 recordStatements로 전후 이벤트를 배출하고, 나머지26회 상한과 알림 INSERT 정확히1회를 각각 검증하도록 테스트만 보완했다. 계측 아래 해당9개는 통과했으며 로컬 전체 게이트와 최신 head CI 결과는 PR 본문에 기록한다.
