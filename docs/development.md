...
| `pnpm db:seed` | 데모 카탈로그 시드. `--scale=small` 로 축소 실행 |
| `pnpm db:seed:verify` | 시드 결과 정합성 점검 (속성·재고 원장·가격 분포) |
...
`pnpm db:seed` 는 판매자 15 · 카테고리 40(3단계) · 상품 800 · 옵션 조합 약 3,600 개를 만든다.
빈 DB 에서 **약 50초**, 다시 돌리면 **1초 미만**이다 — 아무것도 지우지 않고 이미 있는 것을
알아본다. 화면 하나를 보는 동안에는 `pnpm db:seed --scale=small`(상품 50개, 약 30초)이 낫다.

상품·재고는 **API 서비스를 그대로 지나서** 들어간다. 속성 검증과 재고 원장이 실제 요청과
같은 코드를 타므로, 시드가 만든 데이터는 사람이 화면에서 만들 수 있는 데이터와 같다.

`pnpm db:seed:verify` 가 결과를 점검한다 — 개수, 전 상품 속성 유효성, 재고와 원장의 일치,
가격 분포, 브랜드명.
...
실사진을 넣으려면 저장소 루트에 이렇게 둔다. 이 디렉터리는 **커밋하지 않는다**(스톡 라이선스가
재배포를 허용하지 않고, 생성 이미지는 용량이 크다).

```
assets/seed-images/
  stock/<카테고리-slug>/*.jpg      예: stock/women-tops-tshirts/01.jpg
```

넣고 `pnpm db:seed` 를 다시 돌리면 **이미지 URL 만 바뀐다.** 상품·옵션 조합·재고는 그대로다
(실측: 800개 상품과 3,595개 조합과 재고 합계가 모두 불변, 3.3초).
...
testServer.server.use(sleepingInstance(mockPaths.health, 90_000, healthOk))      // 부팅 동안 즉시 거절
testServer.server.use(heldRequestInstance(mockPaths.health, 90_000, healthOk))   // 부팅 동안 요청을 붙잡는다
```

**`sleepingInstance` 가 실측된 Render 의 동작이다** — 부팅이 끝날 때까지 온 요청을 전부 즉시 거절하고,
끝난 다음 요청부터 답한다(아래 ["콜드 스타트"](#콜드-스타트-무료-플랜의-절전)). 거절은 502 응답이 아니라
**전송 실패**로 재현한다: 실제 502 는 CORS 헤더가 없어 브라우저가 앱에 넘기지 않고 `fetch` 를 실패시키기
때문이다. `heldRequestInstance` 는 요청을 붙잡았다가 부팅이 끝나면 답하는 옛 모델이다. 플랫폼이 어느 쪽으로
동작할지는 프론트가 고를 수 없으므로 **재시도 정책은 둘 다에서 검증한다** (TASK-0118 4.6). 두 대역 모두 마감은
요청마다가 아니라 인스턴스마다 하나이고, 첫 요청에서 시계가 돈다.
...
npx --yes js-yaml render.yaml > /tmp/render.json
# --strict=false · ajv-formats: 공식 스키마가 ajv 의 strict 모드와 `uri` 포맷에 걸린다 (2026-09-18 확인)
npx --yes -p ajv-cli@5 -p ajv-formats@2 ajv validate --spec=draft2020 --strict=false \
  -c ajv-formats -s /tmp/render.schema.json -d /tmp/render.json
# 검증이 실제로 도는지 의심되면 plan 을 없는 값으로 바꿔 invalid 가 나오는지 본다
```
...
### 플랜 — 두 서비스 모두 Starter (2026-09-18 · TASK-0145 · D-283)

| 사실 | 결과 |
| --- | --- |
| API · 검색 엔진 모두 **Starter**(0.5 CPU · 512MB). 잠들지 않는다 | 「잠든 뒤 첫 요청 90초」는 평소에는 일어나지 않는다. 아래 "콜드 스타트"는 **배포 · 재기동 구간**의 이야기로 남는다 |
| 검색 엔진에 **1GB 디스크**(`/meili_data`) | 재기동해도 인덱스가 남는다. 자동 재색인(TASK-0038)은 빈 디스크 · 문서 모양 변경 때만 돈다 |
| 디스크가 붙은 서비스는 **무중단 배포가 안 된다** | 검색 엔진을 배포하면 잠깐 닿지 않는다. 이미지 버전이 고정돼 있어 드물고, 그 구간은 화면이 기다린다(TASK-0143) |
| 검색 엔진은 여전히 공개 URL 의 web 서비스다 | 무료 시절의 구조다. 마스터 키 없이는 전부 401 |
| Neon 은 5분 뒤 scale-to-zero | 첫 쿼리가 수백 ms 느리다. 타임아웃을 넉넉히 잡아 뒀다 |

비용은 Starter 월 $7 × 2 + 디스크 약 $0.25, 초 단위 일할이다.

#### `render.yaml` 을 건드리면 sync 가 돈다 — 대시보드에서 바꾼 값이 사라진다

Render 는 연결된 브랜치에 `render.yaml` 변경이 push 되면 **자동으로 Blueprint 를 sync** 하고, 그때 대시보드에서
손으로 바꾼 값을 파일의 값으로 덮는다. **주석만 고친 push 도 그렇다.** 2026-09-17 에 실제로 겪었다 — 대시보드에서
올린 유료 플랜이 `plan: free` 로 되돌아갔고, 검색 엔진이 다시 잠들면서 운영 검색이 500 을 냈다(TASK-0143 · 0145).

- 플랜 · 환경변수(`sync: false` 가 아닌 것)를 대시보드에서 바꾸지 않는다. 바꿨다면 같은 날 `render.yaml` 에 옮긴다
- `render.yaml` 을 고치는 PR 은 주석만 고치더라도 「sync 가 돈다」를 본문에 적는다

#### 검색 엔진의 이미지 버전을 올릴 때

디스크에 옛 버전의 `data.ms` 가 남아 있으면 새 버전이 기동하지 못할 수 있다. 인덱스는 PostgreSQL 의 사본이므로
옮기지 않고 **버린다.** `disk.name` 을 바꾸는 것으로는 안 된다 — Render 는 그것을 새 디스크가 아니라 **기존
디스크의 이름 변경**으로 처리하고 데이터는 남는다. 두 가지 길이 있다.

- 대시보드의 Shell 에서 `rm -rf /meili_data/data.ms` 뒤 Restart — 빈 인덱스에서 자동 재색인이 채운다
- 이미지 태그와 함께 `MEILI_DB_PATH` 를 버전별 경로(예: `/meili_data/v1.25/data.ms`)로 준다 — 선언적이지만
  **아직 해 보지 않았다.** 처음 쓸 때 엔진이 없는 경로를 스스로 만드는지부터 확인한다

### 콜드 스타트 (배포 · 재기동, 그리고 무료 플랜의 절전)

> **2026-09-18 부터 두 서비스는 Starter 라 유휴로 잠들지 않는다 (TASK-0145).** 이 절의 실측과 화면 동작은 무료
> 플랜에서 잰 것이고, 지금은 **배포 · 재기동으로 서비스가 잠깐 닿지 않는 구간**에서 같은 코드가 같은 일을 한다.
> 무료로 되돌리면 이 절이 다시 그대로 맞는다.
...
**API 와 검색 엔진은 각각 따로 뜬다.** 별개의 web 서비스라 무료 플랜에서는 따로 자고, 유료에서도 따로 재기동한다. 반면 **웹 앱 3개(Vercel)는 잠들지
않는다** — 그래서 방문자는 화면을 먼저 보고, 기다리는 것은 데이터 영역뿐이다.

#### 잠든 인스턴스는 요청을 붙잡지 않는다

2026-09-09 실측(TASK-0118 4.1). 잠든 API 로 간 요청은 기다려지지 않고 **바로 거절된다.**

| 상태 | `GET /api/v1/health` |
| --- | --- |
| 잠들어 있거나 부팅 중 | **502 · 0.24 ~ 0.67초.** 본문은 Render 엣지가 만든 HTML 이고 `x-render-routing: dynamic-paid-error` 가 붙는다. 우리 앱을 지나지 않은 응답이라 **CORS 헤더가 없다** |
| 부팅이 끝난 뒤 | 200 · CORS 헤더 정상 |

첫 요청이 기동을 시작시키고, 라우팅이 열릴 때까지(`db:deploy` → 프로세스 기동 → Render 헬스 프로브 통과)
엣지는 계속 502 를 돌려준다. 이 창은 **한 번** 관측됐고 2.7분 이하였다.

위 표의 "약 90초"(2026-09-03)는 `onrender.com` 호스트에서 잰 첫 응답까지의 시간이고, 502 는 커스텀
도메인에서 관측됐다. 두 경로가 다르게 동작하는지는 확인하지 못했다 — 그래서 재시도 정책은 **즉시 거절과
요청 보류 둘 다** 버티게 만들어져 있다.

#### 화면이 하는 일

| 경과 | 구매자 홈 | 판매자 · 관리자 홈 |
| --- | --- | --- |
| 0초 | 헤더·히어로·상품 스켈레톤. 서버 렌더가 API 를 기다리지 않으므로 프리렌더된 HTML 그대로다 | 셸과 상태 패널 스켈레톤 |
| 3초 | 짧은 준비 안내 한 줄 | "서버를 준비하는 중입니다" + 경과 초 + 진행 표시 |
| 15초 | 같은 안내 그대로 | "무료 플랜 데모 환경이라 … 최대 2분까지 걸릴 수 있습니다" |
| 응답 | 자동으로 정상 화면. 누를 것이 없다 | 〃 |
| 150초 동안 실패 | 짧은 실패 안내 + 재시도 버튼 | 실패 사유 + 재시도 버튼. 네트워크가 돌아오거나 탭이 다시 보이면 자동 재시도 |

재시도 예산은 **시도 횟수가 아니라 경과 시간**이다(D-280). 150초 동안, 시도 하나의 마감은 90초, 실패하면
1 → 2 → 4 → 8초를 쉬고 그 뒤로는 8초마다 다시 묻는다 — 콜드 스타트 한 번에 많아야 21회다. 거절이 0.3초에
돌아오므로 횟수로 세면 세 번이 4초 만에 끝나고, 서버는 그 뒤로도 86초를 더 부팅한다. 마감 90초는 플랫폼이
요청을 붙잡는 쪽으로 동작할 때 시도 하나가 부팅을 그대로 받아 내게 하는 값이다. 화면에 시도 번호는 없다.
정책 상수는 `apps/*/src/lib/wake-policy.ts` 에 있고 세 앱이 같은 값이다.

**검색은 따로 표시한다.** 검색 엔진이 아직 깨는 중이거나 색인을 다시 만드는 중이면 헬스 응답의
`search` 가 `ok` 가 아니고, 콘솔은 "검색 준비 중"을 띄운다. 구매자 홈은 준비 안내를 유지하다가 검색이
준비되면 상품을 보여 준다. 빈 결과를 검색 결과처럼 보여 주지 않는다.

전체 시퀀스를 실제 속도로 재현해 볼 수 있다. 즉시 거절과 요청 보류 두 모델을 차례로 돌아 3분쯤 걸린다.
구매자 홈의 안내와, 콘솔의 2단계 안내를 각각 잰다.
...
#### 콘솔에 CORS 에러가 찍힐 때 — 먼저 서버가 자는지 본다

잠든 API 를 부른 브라우저는 **CORS 정책 위반**을 보고한다. 거짓말은 아니다 — 502 HTML 에는 정말로
`Access-Control-Allow-Origin` 이 없다. 다만 그것이 없는 이유가 CORS 설정이 아니라 **헤더를 붙일 앱이 아직
뜨지 않았다**는 것이다. CORS 는 브라우저의 규칙이므로, 오리진 없이 부르면 진짜 상태 코드가 보인다.

```bash
curl -sS -o /dev/null -w '%{http_code} %{time_total}s\n' https://api.<도메인>/api/v1/health
# 502 · 1초 미만 → 서버가 답하지 못하고 있다. CORS 설정을 보지 말 것
# 200            → 서버는 정상이다. 그때 비로소 CORS_ORIGINS 를 의심한다
```

**잠든 것인지 진짜 장애인지는 시간이 가른다.** 위 요청이 이미 기동을 시작시켰으므로 2 ~ 3분 뒤 같은
명령을 한 번 더 부른다.

| 두 번째 결과 | 뜻 |
| --- | --- |
| 200, 응답의 `uptime` 이 작다 | 콜드 스타트였다. 고칠 것이 없다 |
| 여전히 502 | 기동에 실패하고 있다. Render 대시보드의 배포 로그를 본다 — `startCommand` 가 `db:deploy` 를 먼저 돌린다 |

#### cron 으로 계속 깨워 두지 않는 이유 (무료 플랜 시절의 계산)

> 유료 인스턴스는 잠들지 않으므로 지금은 깨울 것이 없다. 아래는 무료 플랜에서 이 규칙을 정한 근거이고,
> 「주기 작업을 늘리지 않는다」는 규칙 자체는 그대로 둔다(TASK-0145 R4).
...
```bash
# 잠들어 있었다면 첫 호출은 502 로 바로 돌아온다 — 그래도 그 호출이 기동을 시작시킨다.
# 90초쯤 뒤에 다시 불러 200 을 확인한다. 응답의 uptime 이 작으면 방금 깨어난 것이다.
