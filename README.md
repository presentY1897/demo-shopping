...
**같은 주문을 세 역할로 보려면 판매자 계정을 먼저 만드세요.** 판매자 데모에는 상품 12개와
체험 주문이 준비된 스토어가 생깁니다. 그 스토어의 상품을 구매한 뒤 같은 주문번호를 판매자·관리자
화면에서 찾습니다. [체험 순서와 확인할 결과](./docs/demo-guide.md)를 따르세요.

API는 무료 인스턴스라 15분 동안 요청이 없으면 잠듭니다. 첫 요청은 최대 90초쯤 걸릴 수 있고,
검색 반영은 비동기라 새로 등록한 상품이 곧바로 검색되지 않을 수 있습니다.
...
운영 환경의 구매 지연은 구간별로 계측한 뒤 줄였습니다. 반복 DB 왕복을 묶고 DB와 API의 리전을
일치시켜, 실제 구매 한 건이 약 17.3초(단회 표본)에서 0.96~1.04초(3회)가 됐습니다.
[계측과 조치 기록](./docs/tasks/M15-polish/TASK-0131-checkout-api-latency-analysis.md)

백엔드는 실제 DB의 제약·잠금·동시 요청을, 프론트는 공통 계약 기반 MSW 테스트로 검증합니다.
[구매·판매자·부분 취소·밀도 전환 E2E](./e2e/tests)도 구성했습니다. 세 역할이 같은 주문을 보는 동선은
[운영 환경에서 따로 검증](./docs/tasks/M15-polish/TASK-0141-cross-role-demo-e2e.md)했습니다. 키보드 조작 등 상세 검증 범위는
...
브랜드명은 모두 가상입니다. 상품 66개의 사진 660장은 **AI로 생성한 콘셉트 이미지**이고,
나머지 시드 상품은 카테고리 색으로 칠한 플레이스홀더입니다. 제작·검수 방식은
[상품 이미지 문서](./product-images/README.md)에 있습니다.
...
| [개발 · 운영 참고](./docs/development.md) | 워크트리·포트, 테스트 하네스, 개발 워크플로, 배포·롤백, 커밋 이력 읽는 법 |
