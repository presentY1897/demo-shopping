 ...
 *
 * The default is `sessionAdminSuper`, because that is the account these screens are
 * ordinarily read by. A spec about a narrower role names it.
 */

import type { SessionResponse } from '@shopping/shared'
import { resetSessionStore, sessionAdminSuper } from '@shopping/api-mocks'
...
export function freshSessionClient(overrides: { readonly now?: () => number } = {}): SessionClient {
  return createSessionClient({ appId: 'admin', baseUrl: BASE_URL, ...overrides })
}

export function renderWithAuth(
  ui: ReactElement,
  { session = sessionAdminSuper }: { readonly session?: SessionResponse | null } = {},
): RenderResult {
