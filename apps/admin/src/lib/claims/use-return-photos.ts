...
/**
 * 반품 사진을 버킷에 올리고, 그 **열쇠**를 신청서에 넘긴다 (TASK-0071 F4).
 *
 ...
 * **아무 봉투도 없다** — 저장소는 우리 API 가 아니므로 403 하나가 전부다.
 *
 * ## 열쇠를 우리가 짓지 않는다
 *
 * 서버가 **부르는 사람**의 id 로 만들어 돌려주고(`UploadsService.keyFor` 가
 * `accountOwnership` 으로 인가한 그 행에서 짓는다), 그래서 관리자가 올린 사진은
 * 언제나 `returns/{관리자 id}/…` 다 — 신청을 내는 사람도 관리자이므로
 * `isOwnPhotoKey` 가 통과한다. 「남의 사진을 이 신청서에 붙이는」 요청은 만들려야
 * 만들 수 없다.
 *
 ...
 * 매달리지 않는다 — 구매자 화면과 판매자 이미지 위젯이 같은 이유로 같은 선택을 했다.
 */
...
export interface ReturnPhotos {
  ...
  /** 신청이 끝나 신청서를 비울 때. 붙인 것을 통째로 버린다. */
  readonly clear: () => void
}
...
export function useReturnPhotos(): ReturnPhotos {
   ...
   *
   * 업로드는 사람이 탭을 옮기거나 신청을 마친 뒤에도 끝날 수 있고, 그때의
 ...
 * 넘기면 그 길이를 정하는 것이 우리가 아니라 실행 환경이 된다.
 */
