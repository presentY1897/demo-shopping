...
import { sessionSellerOwner, sessionSellerApplicant } from '@shopping/api-mocks'
...
function renderGuard(pathname: string, session: MockSession = sessionSellerOwner) {
...
/**
 * `sessionSellerApplicant` is a `BUYER` with a `sellerId` — somebody who applied
 * and was not approved. The console cannot tell that from a rejection, and does
 * not try to (TASK-0023 4장 · R2).
 */
describe('an account without the role (F3 · F7)', () => {
  it('is sent to the refusal screen rather than to sign in again', async () => {
    renderGuard('/orders', sessionSellerApplicant)
...
describe('the refusal screen (F7 · P2)', () => {
  it('says one thing, and does not guess at an application state', async () => {
    renderWithAuth(<NoPermissionScreen messages={auth} />, { session: sessionSellerApplicant })
