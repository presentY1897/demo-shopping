...
import { revenueChartGeometry } from '@/lib/revenue/chart'
import type { RevenueChartMessages } from '@/messages'

/**
 * 매출 추이 — **그림 하나와 표 하나** (TASK-0082 F5).
 *
 * ## 그림은 장식이고 표가 내용이다
 *
 * `<svg>` 에 `aria-hidden` 이 붙어 있고, 같은 숫자가 **진짜 `<table>`** 로 DOM 안에
 * 있다. 흔한 반대 선택 — `<svg role="img" aria-label="매출 추이 차트">` — 은 접근성
 * 검사를 통과하지만 30일치 숫자를 한 마디로 요약해 버린다. 스크린리더로 읽는
 * 판매자에게 「매출 추이 차트」는 **아무 데이터도 아니다.**
 *
 * 표는 접혀 있을 수 있지만 **DOM 에서 사라지지는 않는다.** 접기는 `sr-only` 로
 * 하고 `hidden` 이나 조건부 렌더로 하지 않는다 — 그 둘은 접근성 트리에서도 지워
 * 버리므로, 「접힌 기본 상태」가 곧 「대체 텍스트가 없는 상태」가 된다. 버튼이
 * 바꾸는 것은 **눈에 보이는지**뿐이고, 읽어 주는 쪽에는 언제나 표가 있다.
 *
 * ## 라이브러리가 없다
 *
 * 이유는 `lib/revenue/chart.ts` 의 머리말에 적혀 있다. 여기서는 그 좌표를 받아
 * `<polyline>` 과 `<path>` 두 개로 그린다.
 */
export interface RevenueChartProps {
  readonly days: readonly RevenueDay[]
  readonly messages: RevenueChartMessages
}

export function RevenueChart({ days, messages }: RevenueChartProps) {
  const [open, setOpen] = useState(false)
  const tableId = useId()
  const geometry = revenueChartGeometry(days)

  const columns: readonly TableColumn<RevenueDay>[] = [
    { cell: (row) => day(row.date), header: messages.dateHeader, key: 'date' },
    {
      align: 'end',
    ...
    <section aria-label={messages.title} className="flex flex-col gap-3">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <p className="text-fg-muted text-sm">
          {messages.peak.replace('{amount}', money(geometry.peak))}
        </p>
      </div>
      ...
      {days.length === 0 ? (
        ...
        <>
          ...
          */}
          ...
          {/* 접혀도 여기 있다 — 위 머리말이 왜인지를 적고 있다 (F5). */}
