...
export default defineConfig({
  test: {
    ...
    coverage: {
       ...
       * move a percentage.
       *
       * ## `COVERAGE_THRESHOLDS=off`
       *
       * CI runs this suite in **shards** — a third of the files per runner —
       * and a shard cannot answer these questions. It never loads two thirds of
       * the source, so every file in them reads 0% and every threshold here
       * fails on a run where nothing is wrong. The numbers are checked once, on
       * the merged report (`vitest --merge-reports`), which is the only place
       * that has seen every file.
       *
       * The switch is off by default, so a developer running `pnpm
       * test:coverage` locally gets the gate. It is the shard that opts out,
       * and the job that merges puts it back.
       */
      thresholds:
        process.env.COVERAGE_THRESHOLDS === 'off'
          ...
          : {
              ...
              // TASK-0076 (6.2, Q5 강화). 적립금 원장의 순수 판단: 사건의 부호와 참조,
              // 실결제금액에서 지급액을 내리는 계산, 어느 통에서 얼마를 빼는지, 그리고
              // 원장이 잔액을 아직 설명하는지.
              //
              // 넷 다 빨간 테스트로 나타나지 않는다. 부호가 뒤집힌 행은 잘 만들어진
              // 행이고, 반올림 하나가 뒤집힌 적립은 사람이 세어 보기 전까지 아무도
              // 모르며, 통을 비우지 않은 사용은 **만료가 돌 때** 잔액을 음수로 만들려
              // 든다 — 그때 막는 것은 CHECK 이고, 그 시점에 사람이 보는 것은 원인과
              // 아무 상관 없어 보이는 배치 실패다. 여기서 닿지 않는 분기는 아무도 재지
              // 않는 규칙이다.
              'src/points/point-ledger.ts': {
                branches: 100,
                functions: 100,
                lines: 100,
                statements: 100,
              ...
              /**
               * TASK-0071 (6.2, Q5 강화). 관리자 개입의 순수 판단 — 관리자에게만 열리는
               * 칸, 그가 무시해도 되는 문턱, 무엇을 뒤집을 수 있는가, 그리고 **지연
               * 컷오프**.
               *
               * **넷 다 조용히 틀린다.** 열리는 칸이 한 칸 넓으면 배송 중인 물건이 취소로
               * 들어와 재고가 두 번 늘고, 좁으면 확정 후 하자 반품이 아예 불가능해진다 —
               * 이 TASK 의 절반이 그 한 칸이다. 뒤집을 수 있는 목록이 한 칸 새면 관리자가
               * **아직 아무도 판단하지 않은 신청**을 뒤집고, 그때 원본은 살아 있는 채로
               * 수량을 잡고 있다.
               *
               * 컷오프가 최악이다. 한 칸 늦게 잡으면 **지연된 건이 목록에서 사라지는데**,
               * 그 목록은 밀린 것을 보라고 만든 것이라 비어 있는 것과 없는 것이 화면에서
               * 같아 보인다. 영업일 계산을 SQL 로 내려보내지 않기로 한 대가가 정확히 이
               * 부등식이고, 그것은 분기가 아니라 성질이라 스펙이 한 해를 돌아 확인한다.
               */
              'src/claims/admin-claim-rules.ts': {
                branches: 100,
                functions: 100,
                lines: 100,
                statements: 100,
              ...
              /**
               * TASK-0072 (6.2, Q5 강화). 쿠폰의 순수 판단 — 이 발행을 받아도 되는가,
               * 지금 발급받을 수 있는 기간인가.
               *
               * **세 갈래가 전부 조용히 틀린다.** 판매자 범위가 한 칸 넓으면 판매자가
               * 낸 쿠폰이 남의 가게 매출을 깎고, 그 부담은 정산일까지 아무 데도 나타나지
               * 않는다 — 이 TASK 의 절반이 그 한 칸이다. 정률·정액의 짝이 새면
               * 「150% 할인」이 저장되고 계산기가 그 주문에서 **돈을 돌려준다.** 발급
               * 기간의 경계가 만료 배치와 어긋나면 그 틈에 발급된 쿠폰이 받자마자 만료된
               * 채로 쿠폰함에 앉는다.
               *
               * 셋 다 빨간 검사가 아니라 나중에 금액 하나로 나타난다. 닿지 않는 분기는
               * 아무도 거절하지 않는 조합이라는 뜻이고, DB 의 CHECK 와 짝이 맞는지도 이
               * 갈래들을 전부 밟아야 확인된다.
               */
              'src/coupons/coupon-rules.ts': {
                branches: 100,
                functions: 100,
                lines: 100,
                statements: 100,
              ...
              /**
               * TASK-0072 (6.2, Q5 강화). 쿠폰 코드의 형식과 정규화.
               *
               * **거절하는 갈래가 본업인 모듈이라 100%가 아니면 뜻이 없다.** 통과만 재는
               * 스펙은 「무엇이든 받아 주는 정규화」도 통과시키고, 그 구현은 알파벳 밖
               * 글자를 전부 버려 `WELCOME!!` 같은 입력이 우연히 남의 코드를 가리키게
               * 만든다. 반대로 접기가 빠지면 배너를 보고 `O` 를 옮겨 적은 사람의 절반이
               * 「잘못된 코드입니다」를 받고, 그 화면은 무엇이 틀렸는지 말해 주지 않는다.
               *
               * 생성 쪽도 같다. 나머지 연산에 치우침이 생기면 코드 공간이 실질적으로
               * 좁아지는데, 그것은 어떤 검사도 빨갛게 만들지 않는다.
               */
              'src/coupons/coupon-code.ts': {
                branches: 100,
                functions: 100,
                lines: 100,
                statements: 100,
              ...
              /**
               * TASK-0072 (6.2, Q5 강화). 만료 배치의 상수와 판단.
               *
               * **이 배치가 틀리면 아무것도 실패하지 않는다.** 상한과 주기의 부등식이
               * 뒤집히면 일하느라 늦은 배치가 다음 주기와 겹치고, stale 을 넘기면
               * 헬스체크가 「멈췄다」로 읽는다 — `payment-straggler.ts` 가 같은 이유로
               * 같은 줄에 선다. 로그 판단이 어긋나면 1분마다 「0장」이 쌓여 정작 읽어야 할
               * 한 줄이 그 사이에 묻힌다.
               */
              'src/coupons/coupon-expiry.ts': {
                branches: 100,
                functions: 100,
                lines: 100,
                statements: 100,
              ...
              /**
               * TASK-0075 (6.2, Q5 강화). 쿠폰이 주문서에 닿는지, 얼마를 깎는지.
               *
               * **여기가 틀리면 오류가 아니라 금액으로 나타난다.** 범위 판정이 한 칸
               * 넓으면 판매자 쿠폰이 남의 가게 항목을 깎고, 사는 사람의 화면에는 아무
               * 이상이 없으며, 그 부담은 정산일에 남의 정산액에서 발견된다. 기준 금액이
               * 갈리면 「최소 주문금액은 넘었는데 할인이 0원」이 생기고, 그것을 신고하는
               * 사람은 없다.
               *
               * 추천 조합이 특히 그렇다. 무료배송 판정이 쿠폰까지 반영한 상품금액
               * 기준이므로(`pricing.md` 1장) 할인액만 보고 고르면 **쿠폰을 더 쓰고 더
               * 내는 조합**을 권하게 되는데, 그 갈래는 문턱을 사이에 둔 입력을 실제로
               * 넣어 봐야만 밟힌다.
               *
               * 계산기를 고치지 않는다는 성공 기준(F1)도 이 100%가 지킨다 — 여기서
               * 만들어지는 할인 항목의 세 모양(`ORDER` · `SELLER` · 항목별 `ITEM`)이
               * 전부 밟히지 않으면, 계산기가 받아 본 적 없는 모양이 프로덕션에서 처음
               * 만들어진다.
               */
              /**
               * TASK-0073 · TASK-0074 (6.2, Q5 강화). 쿠폰이 지금 어떤 상태인가.
               *
               * **다섯 갈래가 사람에게 다른 일을 시킨다** — 기다리기 · 다시 열기 · 새로
               * 내기 · 아무것도 못 하기. 겹칠 때 어느 것을 말하느냐가 그 사람이 다음에 하는
               * 일을 정하므로, 가리는 순서가 이 표의 전부다: 끝난 쿠폰을 「중단」이라고
               * 부르면 발행자는 다시 열면 되는 줄 알고 기다린다.
               *
               * 경계도 여기서만 밟힌다. `ENDED` 가 열린 구간인 것은 발급 판정과 같은
               * 순간을 가리켜야 하기 때문이고, 어긋나면 목록이 「진행 중」이라고 적은
               * 쿠폰을 발급 요청이 「기간이 지났다」로 거절한다.
               */
              /**
               * TASK-0078 (6.2, Q5 강화). 환불이 되돌리는 할인.
               *
               * **여기가 틀리면 사람이 손해를 본다, 그리고 그 손해는 조용하다.** 환불은
               * 성공하고 금액도 맞으며, 다만 쓴 적립금이 돌아오지 않거나 쿠폰이 사라진 채
               * 남는다. 반대로 너무 많이 돌려주면 그것은 **없는 돈을 만든 것**이고, 그쪽은
               * 정산일까지 아무 데도 나타나지 않는다.
               *
               * 누계에서 빼는 갈래가 특히 그렇다. 나눠 환불한 사람의 합이 한 번에 환불한
               * 사람과 1원 어긋나는 것은 전량 환불을 한 번 재는 검사로는 절대 밟히지 않는다.
               */
              'src/claims/discount-restore.ts': {
                branches: 100,
                functions: 100,
                lines: 100,
                statements: 100,
              },
              'src/coupons/coupon-console.ts': {
                branches: 100,
                functions: 100,
                lines: 100,
                statements: 100,
              },
              'src/coupons/coupon-apply.ts': {
                branches: 100,
                functions: 100,
                lines: 100,
                statements: 100,
              ...
              /**
               * TASK-0080 (6.2, Q5 강화 — 분기 커버리지 100%). 정산 금액.
               *
               * **이 프로젝트 도메인 설계의 최종 수렴점**이고, 여기가 틀리면 사람이
               * 손해를 보는데 그 손해가 조용하다. 판매자에게 덜 주면 그는 몇 주 뒤
               * 정산서의 숫자 하나로만 그것을 알 수 있고, 더 주면 플랫폼이 받아야 할
               * 것을 못 받는데 그쪽은 아무도 신고하지 않는다.
               *
               * 누계에서 빼는 갈래가 특히 그렇다 — 나눠 반품한 사람의 합이 한 번에
               * 반품한 사람과 1원 어긋나는 것은 전량 반품을 한 번 재는 검사로는
               * 절대 밟히지 않는다.
               */
              'src/settlement/settlement-calc.ts': {
                branches: 100,
                functions: 100,
                lines: 100,
                statements: 100,
