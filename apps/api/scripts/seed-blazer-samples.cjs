// Register the reviewed blazer samples only in the isolated local preview database.
...
const manifestPath = resolve(root, 'apps/shop/public/product-image-sets/blazers-v1/manifest.json')
...
const presentation = {
  menswear: {
    ...
    price: 278900,
    description:
      '노치드 라펠과 두 개의 앞단추로 구성한 레귤러 핏 울 블레이저입니다. 가슴 웰트 포켓과 하단 플랩 포켓, 뒷면의 짧은 중앙 트임을 갖춘 가상 샘플 디자인입니다.',
  },
  womenswear: {
    colors: [
      ['블랙', '#202020', ''],
    ...
    price: 242900,
    description:
      '좁은 노치드 라펠과 브라운 단추 하나로 여미는 레귤러 핏 코튼 블레이저입니다. 하단 플랩 포켓과 뒷면 중앙 트임을 갖춘 가상 샘플 디자인으로, 블랙과 카키를 선택할 수 있습니다.',
  },
}
const labels = {
  front: '상품 앞면',
...
async function main() {
  ...
  try {
    ...
    for (const spec of manifest.spec.products) {
      ...
      const gallery = (color, variant) =>
        ordinary.map((shot) => {
          ...
          return {
            url: `${host}/product-image-sets/blazers-v1/${job.output}`,
      ...
      const input = createProductRequestSchema.parse({
        ...
        skuPrefix: `BLAZER-${spec.audience.toUpperCase()}`,
      })
      const principal = {
    ...
    writeFileSync(
      resolve(root, 'product-images/blazers-products.json'),
