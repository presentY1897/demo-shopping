// Register the reviewed parallel samples only in the isolated local preview database.
...
const lanes = ['a', 'b', 'c']
const manifestPaths = lanes.map((lane) =>
  resolve(root, `apps/shop/public/product-image-sets/parallel-${lane}-v1/manifest.json`),
)
...
const specs = JSON.parse(
  readFileSync(resolve(root, 'product-images/parallel-v1.json'), 'utf8'),
).products
const categorySlugs = {
  가방: 'lifestyle-bags',
...
async function main() {
  ...
  const manifests = manifestPaths.map((manifestPath, index) => {
    ...
    return { lane: lanes[index], data: JSON.parse(readFileSync(manifestPath, 'utf8')) }
  })
  ...
  try {
    ...
    for (const spec of specs) {
      ...
      const lane = sourceManifest.lane
      ...
      const gallery = (color, variant) =>
        ordinary.map((shot) => {
          ...
          return {
            url: `${host}/product-image-sets/parallel-${lane}-v1/${job.output}`,
    ...
    writeFileSync(
      resolve(root, 'product-images/parallel-products.json'),
