// Register the reviewed discovery samples only in the isolated local preview database.
...
const manifestPath = resolve(root, 'apps/shop/public/product-image-sets/discovery-v1/manifest.json')
...
const specs = JSON.parse(
  readFileSync(resolve(root, 'product-images/discovery-v1.json'), 'utf8'),
).products
const categorySlugs = {
  가방: 'lifestyle-bags',
...
async function main() {
  ...
  try {
    ...
    for (const spec of specs) {
      ...
      const gallery = (color, variant) =>
        ordinary.map((shot) => {
          ...
          return {
            url: `${host}/product-image-sets/discovery-v1/${job.output}`,
      ...
      const input = createProductRequestSchema.parse({
        ...
        attributes:
          spec.categoryId === 34
            ? { color: ['아이보리'], material: '면' }
            : spec.categoryId === 35
              ? { color: ['브라운'], material: '가죽' }
              : spec.categoryId === 20
                ? { color: ['그레이'], material: '울', size_free: 'S' }
                : {},
        images,
        options,
    ...
    writeFileSync(
      resolve(root, 'product-images/discovery-products.json'),
