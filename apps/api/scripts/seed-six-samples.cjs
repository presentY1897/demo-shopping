// Register the reviewed six samples only in the isolated local preview database.
...
const lanes = ['a', 'b', 'c']
const manifestFolders = ['six-a-v1', 'six-b-v2', 'six-c-v1']
...
const specs = JSON.parse(readFileSync(resolve(root, 'product-images/six-v2.json'), 'utf8')).products
const categorySlugs = {
  가방: 'lifestyle-bags',
...
async function main() {
  ...
  try {
    ...
    writeFileSync(
      resolve(root, 'product-images/six-products.json'),
