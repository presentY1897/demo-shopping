...
export const permissions = [
  ...
  'question.write',
  /**
   * **자기 알림함**을 읽고 읽음 처리한다 (TASK-0090).
   *
   * 세 앱이 같은 문을 쓴다 — 판매자·관리자 알림도 같은 표에 담기고, 유형에 역할이
   * 묻어 있어서 화면이 자기 것만 그린다. 그래서 이름도 하나다.
   *
   * 스코프는 언제나 `own` 이다. 이 문들은 사용자 id 를 받지 않으므로 「남의 알림함을
   * 연다」가 표현 불가능하다.
   */
  'notification.read',
  /**
   * 신고된 글을 가리거나 지운다 (TASK-0091).
   *
   * `review.moderate` 를 넓히지 않고 새 이름을 둔 이유는 **대상이 넷**이기
   * 때문이다 — 리뷰 · 문의 · 답변 · 상품. 리뷰 이름으로 문의를 가리는 코드는 읽는
   * 사람에게 거짓말을 한다.
   *
   * `DEMO_ADMIN` 은 이것을 `demo` 로 좁혀 받는다. **그것이 D-058 이다** — 방문자의
   * 관리자가 실계정이 쓴 글을 지우지 못하는 자리이고, 거절은 조건문이 아니라 대상의
   * 주인이 누구인가가 만든다.
   */
  /**
   * 신고한다 (TASK-0091).
   *
   * 로그인한 누구나 할 수 있고 스코프는 언제나 `own` 이다 — 이 문은 남의 이름으로
   * 신고할 방법을 갖고 있지 않다. 처리(`content.moderate`)와 나눈 이유는 한 이름으로
   * 묶으면 신고할 수 있는 모든 사람이 남의 글을 가릴 수 있게 되기 때문이다.
   */
  'report.write',
  'content.moderate',
  'review.reply',
