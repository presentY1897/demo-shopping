/**
 * How a failed call becomes words, and whether a reference goes beside them.
 *
 ...
 * error screen.
 *
 * **The catalog below is a fixture, not an app's.** These functions were three
 * copies in `apps/{shop,seller,admin}` until D-219, and the checks that guarded
 * them ran against `apps/admin`'s Korean messages. Tying the common version to
 * one console's wording would make a copy change look like a logic failure, so
 * the sentences here exist only to exercise the branches. Each console still
 * checks its own catalog against these functions — `apps/admin/test/errors.spec.ts`
 * is the one that matters, because it is the only catalog with a placeholder.
 */

import { describe, expect, it } from 'vitest'

import type { ApiFailure } from '../src/index.js'
import {
  ApiClientError,
...
const catalog = {
  errors: {
    CONFLICT: '먼저 바뀐 내용이 있어요.',
    DEPTH: '{max}단계까지만 만들 수 있어요.',
  },
  failures: {
    network: '연결하지 못했어요.',
    timeout: '응답이 오지 않았어요.',
    aborted: '요청을 취소했어요.',
    malformed_response: '응답을 읽지 못했어요.',
    configuration: '주소가 설정되지 않았어요.',
...
describe('interpolate', () => {
  it('leaves a template with no placeholder alone', () => {
    // This is the branch that lets `failureMessage` interpolate unconditionally
    // (D-219): two of the three catalogs carry no `{자리}` at all, so for them
...
describe('errorMessage', () => {
  it('answers with the catalog’s sentence for a code it knows', () => {
    expect(errorMessage(catalog.errors, 'CONFLICT')).toBe(catalog.errors.CONFLICT)
  })

  it('answers nothing for a code from the future, so the caller can fall back', () => {
    expect(errorMessage(catalog.errors, 'CODE_NOBODY_DECLARED')).toBeUndefined()
  })

  it('answers nothing when the sentence needs a value that did not arrive', () => {
    expect(errorMessage(catalog.errors, 'DEPTH')).toBeUndefined()
    expect(errorMessage(catalog.errors, 'DEPTH', { max: 3 })).toBe('3단계까지만 만들 수 있어요.')
  })
})
...
describe('classifying what was thrown', () => {
  it('recognises a missing API URL', () => {
    // One class, in this package, precisely so `instanceof` works: three apps
    // declaring their own would be three different types to this check (D-219).
  ...
  it('keeps everything the envelope carried', () => {
    const failure = apiFailure(
      httpError({
        ...
        code: 'CONFLICT',
        message: '서버 문장',
        details: [{ field: 'slug', message: '서버 문장', code: 'CONFLICT' }],
        requestId: 'req-42',
      }),
    )
  ...
  it('survives a client error with no status of its own', () => {
    expect(apiFailure(httpError({ code: 'CONFLICT' }))).toMatchObject({ status: 0 })
  })
})

describe('hasCode', () => {
  it('is false for a failure that never reached the API', () => {
    expect(hasCode({ kind: 'transport', reason: 'network' }, 'CONFLICT')).toBe(false)
  })

  it('separates two codes that share a status', () => {
    const conflict = apiFailure(httpError({ status: 409, code: 'CONFLICT' }))
    const taken = apiFailure(httpError({ status: 409, code: 'SKU_TAKEN' }))

    expect(hasCode(conflict, 'CONFLICT')).toBe(true)
    expect(hasCode(taken, 'CONFLICT')).toBe(false)
  })
})
...
describe('failureMessage', () => {
  it('uses the transport catalog when nothing arrived', () => {
    ...
    expect(failureMessage(failure, catalog)).toBe(catalog.failures.timeout)
  })

  it('uses the app’s sentence, not the server’s', () => {
    const failure = apiFailure(
      httpError({ status: 409, code: 'CONFLICT', message: '이미 사용 중인 슬러그입니다.' }),
    )

    expect(failureMessage(failure, catalog)).toBe(catalog.errors.CONFLICT)
  })
  ...
  it('interpolates from the values the failure carried', () => {
    ...
    expect(failureMessage(failure, catalog)).toBe('3단계까지만 만들 수 있어요.')
  })

  it('falls back to the server’s sentence when the values never came', () => {
    // Better a server-worded sentence than `{max}` rendered at an operator —
    // which is what an optional interpolation argument would have shipped the
    // first time a call site forgot it (D-219).
    const failure = apiFailure(
      httpError({ status: 400, code: 'DEPTH', message: '서버만 아는 문장' }),
    )
...
describe('quotableRequestId', () => {
  ...
  it('offers nothing for a failure the reader can act on', () => {
    const failure = apiFailure(httpError({ status: 409, code: 'CONFLICT', requestId: 'req-7' }))
