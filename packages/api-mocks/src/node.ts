...
export function setupTestServer(...extraHandlers: readonly RequestHandler[]): TestServer {
  ...
  afterEach(() => {
    ...
    // 여부를 정한다.**
    resetSellerClaimStore()
    // 판매자 콘솔의 쿠폰 (TASK-0074). 한 스펙이 중단한 쿠폰이나 새로 발행한 쿠폰이
    // 넘어가면 다음 스펙의 「여섯 줄」이 앞 스펙의 「일곱 줄」이 되고, 상태 필터를
    // 재는 검사는 **파일 안의 순서에 따라** 통과하거나 실패한다.
    resetSellerCouponStore()
    ...
    resetSellerStore()
    // The account screens edit a profile, settings and an address book; a row
    // that survived into the next test would make specs pass or fail by their
    // order in the file, exactly as a created category would.
    resetProfileStore()
